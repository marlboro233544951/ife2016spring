...
function addAqiData() {
	...
	console.log(city.match(/^[A-Za-z\u4E00-\u9FA5]+$/));
...
function init() {
  ...
  document.getElementById("aqi-table").addEventListener("click", function(event){
        if(event.target.nodeName.toLowerCase() === 'button') delBtnHandle(event.target.dataset.city);//使用delBtnHandle.call(null,event.target.dataset.city)
    })
}
